...
export default function Borrows() {
  const { data, addBorrow, returnBorrow, deleteBorrow } = useData();
    ...
    <div>
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-4 flex flex-wrap gap-3 items-center justify-between">
        <label className="flex items-center gap-2 text-sm cursor-pointer">
          <input type="checkbox" checked={activeOnly} onChange={() => setActiveOnly(!activeOnly)} />
          Show active only
        </label>
        <button
          className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition text-sm disabled:opacity-50"
          onClick={openModal}
          disabled={availableBooks.length === 0 || activeMembers.length === 0}
        >
          <i className="fas fa-handshake mr-1" /> Borrow Book
        </button>
      </div>
                ...
                {filteredBorrows.map(br => {
                  ...
                  return (
                    <tr key={br.id} className="border-b hover:bg-slate-50">
                      <td className="px-4 py-3 max-w-[150px] truncate" title={getBookTitle(br.bookId)}>{getBookTitle(br.bookId)}</td>
                      <td className="px-4 py-3">{getMemberName(br.memberId)}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs ${br.status === 'borrowed' ? (overdue ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') : 'bg-green-100 text-green-700'}`}>
                          {br.status === 'borrowed' ? (overdue ? 'Overdue' : 'Borrowed') : 'Returned'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right space-x-1">
                        {br.status === 'borrowed' && (
                          <button className="text-green-500 hover:text-green-700 text-xs bg-green-50 px-2 py-1 rounded" onClick={() => handleReturn(br.id)}>
                            <i className="fas fa-undo mr-1" /> Return
                          </button>
                        )}
                        <button className="text-red-500 hover:text-red-700 text-xs bg-red-50 px-2 py-1 rounded" onClick={() => handleDelete(br.id)}>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title="Borrow Book">
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          <button className="px-4 py-2 rounded-lg bg-green-500 text-white hover:bg-green-600 text-sm" onClick={handleBorrow} disabled={!form.bookId || !form.memberId}>
            <i className="fas fa-handshake mr-1" /> Borrow
          </button>
        </div>
      </Modal>
    </div>
